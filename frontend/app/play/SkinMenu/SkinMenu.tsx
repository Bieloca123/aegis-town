...
import { skins, skinIds, defaultSkin } from '@/utils/pixi/Player/skins'
...
const SkinMenu:React.FC<SkinMenuProps> = () => {
    ...
    const [skinIndex, setSkinIndex] = useState<number>(skinIds.indexOf(defaultSkin))
    ...
    useEffect(() => {
        const onGotSkin = (skin: string) => {
            setSkinIndex(skinIds.indexOf(skin))
        }
    ...
    async function switchSkins() {
        const newSkin = skins[skinIndex].id
        ...
        <Modal open={modal === 'Skin'} closeOnOutsideClick>
            <div className='w-96 h-96 flex flex-col items-center justify-between pt-8'>
                ...
                <div className='flex flex-col items-center gap-2'>
                    <AnimatedCharacter src={`/sprites/characters/Character_${currentSkin.id}.png`} className='w-48'/>
                    {currentSkin.name && (
                        <div className='text-center'>
                            <p className='font-bold text-lg leading-tight'>{currentSkin.name}</p>
                            {currentSkin.theme && (
                                <p className='text-xs opacity-70'>{currentSkin.theme}</p>
                            )}
                        </div>
                    )}
                </div>
